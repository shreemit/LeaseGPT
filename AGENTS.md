...
- Refresh listings only via `uv run python scripts/fetch_rentcast_listings.py` with `RENTCAST_API_KEY` in the environment or a gitignored `.env` at the repo root or `leasegpt/.env`. Never import that script from the app. The script updates existing ids, appends new ones, and drops missing ids only after a complete API page walk.
- The checked-in JSONL is RentCast API Data (display/distribution allowed under their API ToS §1). It is not MIT-licensed. Keep contact redaction. Do not check in `RENTCAST_API_KEY`.
...
UI: `layout="centered"`, tabs **Chat / Sources / Listings**. Sidebar is the Groq key only. Price/neighborhood filters live on **Listings** and affect sample cards only (the tab shows at most `LISTINGS_CARD_LIMIT` cards); retrieval always searches the full index. Chat expander (“Why this answer”) lists listing titles; Sources shows retrieved chunks.
...
- Embeddings: local FastEmbed ONNX. Process-level cache in `retriever._vector_store`. Optional gitignored FAISS at `data/faiss/` (built with `uv run python scripts/build_vector_store.py`); the app loads it when `stamp.json` matches the snapshot SHA-256 digest, listing count, fetch date, and FastEmbed model, otherwise it builds on first use. Do not check in the index.
